...
<?php
		...
		$result = mysqli_query($con, "select * from tb_ders where ID = '". $_GET["ders"]. "'" );
...
if ($_SERVER["REQUEST_METHOD"] == "POST"){ 
	...
	if ($_POST["sil"] != NULL){  // Sil Tıklanmışsa bul ve sil 
		...
		mysqli_query($con,"DELETE FROM tb_ders WHERE ID = ". $_POST["dd"]);
		mysqli_close($con);
		...
		mysqli_query($con,"UPDATE tb_ders SET grup = 0 , baslik = '". $_POST["baslik"]. "' , aktif = ". $aktif .", sinif = '".$_POST["sinif"] ."' , text = '". $_POST["content"]. "' WHERE ID = ". $_POST["dd"]);
