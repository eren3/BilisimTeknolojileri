...
if ($_GET["id"]!= NULL){
		$result = mysqli_query($con, "select * from tb_soru where ID = ". $_GET["id"] );
		...
		$result2 = mysqli_query($con, "select * from tb_secenek where soruID = ". $IDs );
		$i=0;
		...
		mysqli_query($con,"DELETE FROM tb_soru WHERE ID = ". $_GET["id"]);
		mysqli_query($con,"DELETE FROM tb_secenek WHERE ID = ". $ID[0]);
		mysqli_query($con,"DELETE FROM tb_secenek WHERE ID = ". $ID[1]);
		mysqli_query($con,"DELETE FROM tb_secenek WHERE ID = ". $ID[2]);
		mysqli_query($con,"DELETE FROM tb_secenek WHERE ID = ". $ID[3]);
		mysqli_query($con,"DELETE FROM tb_secenek WHERE ID = ". $ID[4]);
		mysqli_close($con);
...
<form action="" method="post" name="form1" id="form1">
  ...
  <a href="derssayfa.php?id=<?php echo $_GET["dersID"]; ?>">İptal
...
<p>
 ...
 if ($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		mysqli_query($con,"INSERT INTO tb_soru (grup, text, puan) VALUES (" . $_GET["dersID"]." , '". $_POST["textarea"]. "', " . $_POST["puan"] . " )" );

	
		$result = mysqli_query($con, "select * from tb_soru" ); 
		...
		*/
		...
		mysqli_query($con,"INSERT INTO tb_secenek (soruID, text, dogrumu) VALUES (" . $IDson .",'". $_POST["t3"]. "'," . $rr . ")" );
		
		
			if ($_POST["radio"]==3){ $rr=1; }else { $rr =0;}
		mysqli_query($con,"INSERT INTO tb_secenek (soruID, text, dogrumu) VALUES (" . $IDson .",'". $_POST["t4"]. "'," . $rr . ")" );
		
		
			if ($_POST["radio"]==4){ $rr=1; }else { $rr =0;}
		mysqli_query($con,"INSERT INTO tb_secenek (soruID, text, dogrumu) VALUES (" . $IDson .",'". $_POST["t5"]. "'," . $rr . ")" );
		
		
			if ($_POST["radio"]==5){ $rr=1; }else { $rr =0;}
		mysqli_query($con,"INSERT INTO tb_secenek (soruID, text, dogrumu) VALUES (" . $IDson .",'". $_POST["t6"]. "'," . $rr . ")" );
		
}  else // Yeni kayıt ekleme SONU 
{		mysqli_query($con,"UPDATE tb_soru SET text = '". $_POST["textarea"]. "', puan = ". $_POST["puan"] . " WHERE ID =" . $_GET["id"] );
		
		
			if ($_POST["radio"]==1){ $rr=1; }else { $rr =0;}
		mysqli_query($con,"UPDATE tb_secenek SET text = '". $_POST["t2"]. "', dogrumu = " . $rr . " WHERE ID =". $ID[0] );
				
			if ($_POST["radio"]==2){ $rr=1; }else { $rr =0;}
		mysqli_query($con,"UPDATE tb_secenek SET text = '". $_POST["t3"]. "', dogrumu = " . $rr . " WHERE ID =". $ID[1] );
				
			if ($_POST["radio"]==3){ $rr=1; }else { $rr =0;}
		mysqli_query($con,"UPDATE tb_secenek SET text = '". $_POST["t4"]. "', dogrumu = " . $rr . " WHERE ID =". $ID[2] );
				
			if ($_POST["radio"]==4){ $rr=1; }else { $rr =0;}
		mysqli_query($con,"UPDATE tb_secenek SET text = '". $_POST["t5"]. "', dogrumu = " . $rr . " WHERE ID =". $ID[3] );
			
			if ($_POST["radio"]==5){ $rr=1; }else { $rr =0;}
		mysqli_query($con,"UPDATE tb_secenek SET text = '". $_POST["t6"]. "', dogrumu = " . $rr . " WHERE ID =". $ID[4] );
